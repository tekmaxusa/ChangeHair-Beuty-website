...
// --- Components --- (site nav lives in `SalonHeader` via `SalonLayout`)
...
export const SalonMenuView = ({ onBack, onOpenBooking }: { onBack: () => void; onOpenBooking: (categoryId: string) => void }) => (
  <div className="min-h-screen bg-salon-beige pt-28 pb-32 md:pb-40">
...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrorMessage('');
    try {
      const tok = await apiJson<{ ok?: boolean; csrf?: string }>('/api/contact/token.php', { method: 'GET' });
      const csrf = tok.csrf;
      if (!csrf) {
        throw new Error('Could not start contact session.');
      }
      const res = await apiFetch('/api/contact/submit.php', {
        method: 'POST',
...
export default function HomePage() {
  ...
  const openBooking = useCallback(
    (categoryId?: string) => {
      navigate('/booking', categoryId ? { state: { preselectCategory: categoryId } } : undefined);
    },
    [navigate],
  );
  ...
  useEffect(() => {
    ...
    const st = location.state as { view?: string; openBooking?: boolean } | null;
    if (!st || (!st.view && !st.openBooking)) {
      return;
    }
    if (st.openBooking) {
